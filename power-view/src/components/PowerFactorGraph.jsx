...
const PowerFactorGraph = ({meter, interval}) => {
    ...
    React.useEffect( () => {
        axios.post(`http://localhost:3000/samples/powerfct/${meter}`, interval).then ( res => {
    ...
    const yyScale = scaleLinear({
      domain: [0, Math.max(...powerSamples.map(d => d.power_factor))],
      range: [yMax, 0],
    });         
  ...
  return (
    <svg width={width} height={height}>
      <Group left={margin.left} top={margin.top}>
        ...
        <AxisLeft scale={yyScale} top={0} left={0} label="Power Factor" />
